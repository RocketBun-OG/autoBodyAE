...
bodypreset FindPresetByName(std::vector<bodypreset> searchable,
                            std::string name) {
...
bool isInINI(RE::Actor *a_actor, bool racesex, bool faction) {
  ...
  if (faction) {
    ...
    for (RE::FACTION_RANK rank : ranklist) {
      ...
      for (categorizedList fac : faclist) {
        ...
        logger::trace("{} is what we're looking at in the faction loop",
                      fac.faction);
  ...
  for (categorizedList list : charlist) {
    ...
    if (actorID == list.formID) {
      logger::trace("{} found a matching config in the INI!",
                    a_actor->GetName());
      found = true;
    }
  }
...
RE::BSEventNotifyControl HandleGeneration(RE::Actor *actor, bool genOverride) {
  ...
  auto keywordNPC =
      defaultobj->GetObject<RE::BGSKeyword>(RE::DEFAULT_OBJECT::kKeywordNPC);
  ...
  if (actor->HasKeyword(keywordNPC)) {
    ...
    if (!morphman->IsGenned(actor) || !genOverride) {
      ...
      if (actor->formID == playerID) {
        morphman->morphInterface->SetMorph(actor, "nobody_processed", "NoBody",
                                           1.0f);
        logger::trace(
            "We've detected the player. Skipping preset application.");
      ...
      logger::trace("Actor has already had a preset applied. Skipping preset "
                    "application.");
    }
  }
  return RE::BSEventNotifyControl::kContinue;
...
void PrintPresetList(std::vector<bodypreset> setofsets) {
  ...
  for (bodypreset body : setofsets) {
    logger::info("[ {} ] Number of sliders: [ {} ]", body.name,
                 body.sliderlist.size());
  }
}
...
void ParsePreset(std::string filename, std::vector<bodypreset> *list) {
  ...
  for (xml_node<> *preset_node = root_node->first_node("Preset"); preset_node;
       preset_node = preset_node->next_sibling()) {
    ...
    *presetname = preset_node->first_attribute()->value();
    ...
    if (presetname->find("Cloth") != -1 || presetname->find("cloth") != -1 ||
        presetname->find("Outfit") != -1 || presetname->find("outfit") != -1) {
    ...
    for (xml_node<> *slider_node = preset_node->first_node("SetSlider");
         slider_node; slider_node = slider_node->next_sibling()) {
      ...
      *sizevalue =
          std::stoi(slider_node->first_attribute("value")->value()) / 100.0f;
      ...
      // full struct with null values where they belong.
               ...
               slider_node->next_sibling()) {
        if (slider_node->next_sibling()->first_attribute()->value() !=
            *slidername) {
          if (*size == "big") {
    ...
    logger::info("{} has just been ingested into the master preset list.",
                 *presetname);
...
std::string CheckConfig() {
  ...
  if (rc < 0) {
    logger::critical(
        "Config INI not found! Please make sure it is in /Data/Nobody/Config/");
  }
  ...
  case 1:
    morf->biasamount =
        atof(configINI.GetValue("Features", "bWeightBiasAmount"));
  default:
...
void CheckMorphConfig() {
  ...
  if (rc < 0) {
    ...
    if (rc < 0) {
      logger::critical("Bodygen INI not found! Please make sure it is in "
                       "/Data/Nobody/Config/");
    }
  }
  ...
  for (std::list<CSimpleIniA::Entry>::iterator keylistIter = keylist.begin();
       keylistIter != keylist.end(); keylistIter++) {
    ...
    if (name.find("Female") != -1) {
      ...
      // faction and race.
        ...
        logger::trace(
            "The category is not a faction or a race. It is being ignored.");
      }
    ...
    // no men allowed lul
      ...
      logger::info("Male bodies are currently not supported. the presets for "
                   "{} will not load.",
                   name);
    }
    ...
    if (notempty) {
      ...
      if (parsedlist.formID != 0) {
        logger::trace("formID {} is being passed into the character list!",
                      name);
      ...
      // if its a race preset, it goes into the race list.
        ...
        logger::trace("{} is being passed to the raceSex list!",
                      parsedlist.race);
      ...
      // if it's a faction preset, it goes into the faction list.
        ...
        logger::trace("{} is being passed to the faction list!",
                      parsedlist.faction);
...
void SetConfigKey(const char *sectionname, const char *keyname,
                  const char *value) {
  CSimpleIniA configINI;
  SI_Error rc = configINI.LoadFile("./Data/Nobody/Config/NobodyConfig.ini");
  if (rc < 0) {
